...
> [!IMPORTANT]
> On every step, if you have a [custom domain](https://auth0.com/docs/customize/custom-domains), replace the `YOUR_AUTH0_DOMAIN` and `dev-r1s7xzqfu3rt2lmr.us.auth0.com` placeholders with your custom domain instead of the value from the settings page.
...
Rename `.env.example` to `.env` and replace the `aG1x463yGHmdpoFWF3KAVXNkGxRaX0TJ` and `dev-r1s7xzqfu3rt2lmr.us.auth0.com` placeholders with the Client ID and domain of your Auth0 application.
...
In the sample, we are using values referenced from `android/app/src/main/res/values/strings.xml`. Rename `strings.xml.example` to `strings.xml` and replace the `dev-r1s7xzqfu3rt2lmr.us.auth0.com` placeholder with the domain of your Auth0 application.

```xml
<?xml version="1.0" encoding="utf-8"?>
<resources>
    <string name="com_auth0_domain">dev-r1s7xzqfu3rt2lmr.us.auth0.com</string>
